...
def _merge_key_for_record(rec: dict, *, date_aware: bool = False) -> str:
    """Build the identity key for merging records from different sources."""
    fn = rec.get("flight_number", "")
    if not fn:
        return ""
    parts = [fn]
    if date_aware:
        parts.append(_extract_date(rec.get("scheduled_departure")))
    parts.extend([
        rec.get("origin_iata", "") or "",
        rec.get("destination_iata", "") or "",
    ])
    return "|".join(parts)
...
def _merge_records(records: list[dict], *, date_aware: bool = False) -> list[dict]:
    ...
    for rec in sorted_records:
        ...
        key = _merge_key_for_record(rec, date_aware=date_aware)
    ...
    for merged_rec in result:
        ...
        if not fn:
            continue
        group_key = _merge_key_for_record(merged_rec, date_aware=date_aware)
...
def _query_keys(query: str) -> tuple[str, str]:
    q = (query or "").strip()
    q_lower = q.lower()
    q_nospace = re.sub(r"\s+", "", q_lower)
    return q_lower, q_nospace


def _is_exact_airport_query(query: str, resolved: list[str]) -> bool:
    """Return True when the user's input names a specific airport."""
    if len(resolved) != 1:
        return False
    q = (query or "").strip()
    if not q:
        return False
    q_lower, q_nospace = _query_keys(q)

    if q_nospace in getattr(airport_manager, "_iata_index", {}):
        return True
    alias_index = getattr(airport_manager, "_alias_index", {})
    if q_lower in alias_index or q_nospace in alias_index:
        return True
    for alias, iata in alias_index.items():
        if iata in resolved and alias and (alias in q_lower or alias in q_nospace):
            return True
    for name, iata in getattr(airport_manager, "_name_cn_index", {}).items():
        if iata in resolved and (q in name or name in q):
            return True
    return False


def _fliggy_query_value(query: str, resolved: list[str]) -> str:
    """Preserve exact airport input for Fliggy; use city names only for city queries."""
    if _is_exact_airport_query(query, resolved):
        return resolved[0]
    return _iata_to_city_cn(resolved[0])


def _build_route_constraints(
    origin_query: str,
    destination_query: str,
    origins: list[str],
    dests: list[str],
) -> dict:
    """Build airport constraints from the user's original route input."""
    exact_origin = _is_exact_airport_query(origin_query, origins)
    exact_dest = _is_exact_airport_query(destination_query, dests)
    return {
        "origin_query": origin_query,
        "destination_query": destination_query,
        "exact_origin": exact_origin,
        "exact_destination": exact_dest,
        "origin_allowed": set(origins) if exact_origin else None,
        "destination_allowed": set(dests) if exact_dest else None,
    }


def _constraints_to_json(constraints: dict) -> dict:
    """Convert route constraints to JSON-serialisable data."""
    return {
        "origin_query": constraints.get("origin_query"),
        "destination_query": constraints.get("destination_query"),
        "exact_origin": constraints.get("exact_origin", False),
        "exact_destination": constraints.get("exact_destination", False),
        "origin_allowed": sorted(constraints.get("origin_allowed") or []),
        "destination_allowed": sorted(constraints.get("destination_allowed") or []),
    }


def _constraint_rejection_reason(record: dict, constraints: dict) -> str:
    origin_allowed = constraints.get("origin_allowed")
    destination_allowed = constraints.get("destination_allowed")
    reasons = []
    if origin_allowed is not None:
        origin = record.get("origin_iata", "")
        if origin not in origin_allowed:
            reasons.append(
                f"origin {origin or 'UNKNOWN'} not in requested {sorted(origin_allowed)}"
            )
    if destination_allowed is not None:
        dest = record.get("destination_iata", "")
        if dest not in destination_allowed:
            reasons.append(
                f"destination {dest or 'UNKNOWN'} not in requested {sorted(destination_allowed)}"
            )
    return "; ".join(reasons)


def _record_matches_route_constraints(record: dict, constraints: dict) -> bool:
    return not _constraint_rejection_reason(record, constraints)


def _split_records_by_route_constraints(
    records: list[dict],
    constraints: dict,
    source_name: str = "",
) -> tuple[list[dict], list[dict]]:
    """Split records into route-valid main results and airport-mismatch fallback."""
    if not constraints.get("origin_allowed") and not constraints.get("destination_allowed"):
        return records, []

    valid = []
    fallback = []
    for rec in records:
        reason = _constraint_rejection_reason(rec, constraints)
        if not reason:
            valid.append(rec)
            continue
        fallback_rec = rec.copy()
        fallback_rec["_constraint_status"] = "fallback"
        fallback_rec["_constraint_reason"] = reason
        if source_name:
            fallback_rec["_constraint_source"] = source_name
...
def _execute_concurrent_queries(
    ...
    Args:
        ...
        record_filter: optional callable(records, source_name) returning
            (valid_records, fallback_records).  It is applied before smart
            return and sufficient-source checks.
        fallback_results: optional list to receive filtered-out records.
    ...
    while pending:
        ...
        for future in done:
            ...
            try:
                ...
                fallback = []
                if record_filter:
                    data, fallback = record_filter(data, name)
                    if fallback:
                        logger.warning(
                            "%s: %d result(s) excluded by route constraints",
                            name, len(fallback),
                        )
                        if fallback_results is not None:
                            fallback_results.extend(fallback)
                results.extend(data)
...
def cmd_search(args):
    ...
    dest_str = ",".join(dests)
    constraints = _build_route_constraints(
        args.from_station, args.to_station, origins, dests
    )
    ...
    if fg_cfg.get("enabled", False) and args.date:
        import functools
        fg_origin = _fliggy_query_value(args.from_station, origins)
        fg_dest = _fliggy_query_value(args.to_station, dests)
    ...
    results = _execute_concurrent_queries(
        ...
        record_filter=lambda data, name: _split_records_by_route_constraints(
    ...
    fallback_merged = _merge_records(fallback_results)

    if not getattr(args, "show_codeshare", False):
        merged = _deduplicate_codeshares(merged)
        fallback_merged = _deduplicate_codeshares(fallback_merged)
    ...
    if getattr(args, "layover_max_hours", None) is not None:
        ...
        fallback_merged = [r for r in fallback_merged
                           if r.get("stops", 0) == 0
                           or r.get("max_layover_minutes", 0) <= max_mins]
    ...
    _apply_currency_conversion(merged, args, conf)
    _apply_currency_conversion(fallback_merged, args, conf)

    if args.output == "json":
        if getattr(args, "include_fallback", False):
            payload = {
                "flights": merged,
                "fallback_flights": fallback_merged,
                "query_constraints": _constraints_to_json(constraints),
                "source_warnings": [
                    "Some source results were excluded because they did not match exact airport constraints."
                ] if fallback_merged else [],
            }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    search_p.add_argument(
        "--include-fallback", action="store_true", default=False,
        dest="include_fallback",
        help="With JSON output, include airport-mismatch fallback records and constraints",
    )
